...
class UseCaseViewController: UIViewController , UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIActionSheetDelegate,  UITableViewDelegate, UITableViewDataSource{
    ...
    var selectedUseCaseList : Array<String> = []
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        Mint.sharedInstance().logViewWithCurrentViewName("UseCaseViewController", limitedExtraDataList: datalist)
    ...
    func initHelpers(){
        ...
        self.useCaseList =   Constants.sharedInstance.useCaseList
    ...
    func drawUserInterface(){
        ...
        top += 20.0
        scrollView.addSubview(createLabel("Select All Relevant Use Case(s)", frame: CGRectMake(leftOffset, top, fullWidth, 16), align:1))
        
        
        /// var optionTableVIew: UITableView!
        top += 40
        //TableView
        self.tableView =  createTableView(CGRectMake(leftOffset, top, fullWidth , self.view.frame.size.height - 240))
        ...
        top += self.view.frame.size.height - 240 + 10
    ...
    func checkForm(tag: Int) -> Bool{
        ...
        if(self.selectedUseCaseList.count == 0 )
        {
            errors.append("Please select at least one use case")
        }
    ...
    @IBAction func savePressed(sender: UIButton){
        ...
        var isFirst = 0
        var useCase = ""
        for tempUseCase in selectedUseCaseList
        {
            if(isFirst == 1)
            {
                useCase = "\(useCase),\(tempUseCase)"
        ...
        Constants.sharedInstance.leadData.use_case = useCase
        
        println("useCase: \(useCase)")
         println("use_case: \(Constants.sharedInstance.leadData.use_case)")
        
        self.showCompetitionViewController()
        
    }
    
    ///
    func showCompetitionViewController()
    {
        println("showCompetitionViewController")
        
        let appDelegate = UIApplication.sharedApplication().delegate as! AppDelegate
        
        let storyBoard = UIStoryboard(name: "Main", bundle: nil)
        
        let competitionViewController = storyBoard.instantiateViewControllerWithIdentifier("competitionViewController") as! CompetitionViewController
        
        self.navigationController!.pushViewController(competitionViewController, animated: true)
    }
    
    
    //Table View Area
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return useCaseList.count
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let useCase = self.useCaseList[indexPath.row]
            as String
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        let useCase = self.useCaseList[index]
            as String
        
        var position = 0
        var isFound = false
        for tempUseCase in self.selectedUseCaseList
        {
            if(tempUseCase == useCase)
            {
                // position++
                ...
                isFound = true
                selectedUseCaseList.removeAtIndex(position)
        ...
        if(!isFound)
        {
            selectedUseCaseList.append(useCase)
        }
    ...
    func getAllUseCaseData(){
        
        //    eventcollection, usecasecollection, competitioncollection, shirtslogancollection, shirtsizecollection
        
        ///
        var basicUrl =   "https://INDEXER:8089/servicesNS/nobody/splunkt_splunk_app/storage/collections/data/usecasecollection"
        ...
        client.GET(basicUrl, parameters: params, success: { (operation : AFHTTPRequestOperation!, response : AnyObject!) -> Void in
            ...
            Constants.sharedInstance.useCaseList = []
            ...
            if(jsonDict.count > 0)
            {
                ...
                for var i=0 ;i < jsonDict.count; i++ {
                    ...
                    var addUseCase = row.valueForKey("addUseCase") as! String!
                    
                    Constants.sharedInstance.useCaseList.append(addUseCase)
