...
#import "BazarTextField.h"

@interface BazarTextField()
{
...
- (void)markTextFieldsWithTagInView:(UIView*)view{
    ...
    if ([self.textFields count] == 0){
        for(UIView *subView in view.subviews){
            if ([subView isKindOfClass:[BazarTextField class]] || [subView isKindOfClass:[UITextView class]]){
                BazarTextField *textField = (BazarTextField*)subView;
                textField.tag = index;
...
- (void) nextButtonIsClicked:(id)sender{
    ...
    BazarTextField *textField =  [self.textFields objectAtIndex:++tagIndex];
...
- (void) keyboardDidShow:(NSNotification *) notification{
    ...
    if (![_textField isKindOfClass:[BazarTextField class]]) return;
